...
//
//  Tests for TabGroupService business logic.
...
struct TabGroupServiceTests {
    ...
    func makeService() -> (service: TabGroupService, repo: MockGroupRepository, tabManager: MockTabManager) {
        let repo = MockGroupRepository()
        let tabManager = MockTabManager.withSampleTabs()
        let service = TabGroupService(repository: repo, tabManager: tabManager)
    ...
    func createGroupThrowsOnEmptyName() async throws {
        ...
        await #expect(throws: TabGroupError.self) {
    ...
    func deleteGroupRemovesGroup() async throws {
        ...
        try await service.deleteGroup(groupID: group.id, windowID: windowID)
    ...
    func deleteGroupThrowsOnNonExistentGroup() async throws {
        ...
        await #expect(throws: StorageError.self) {
            try await service.deleteGroup(groupID: UUID(), windowID: windowID)
        }
    }
    ...
    func getAllGroupsReturnsAllGroups() async throws {
        let (service, _, _) = makeService()
        let windowID = "window-1"
        ...
        let groups = try await service.getAllGroups(windowID: windowID)
    ...
    func addTabToGroupAddsTabToGroup() async throws {
        ...
        try await service.addTabToGroup(tabID: "tab-1", groupID: group.id, windowID: windowID)

        let updatedGroups = try await service.getAllGroups(windowID: windowID)
    ...
    func removeTabFromGroupRemovesTabFromGroup() async throws {
        ...
        try await service.removeTabFromGroup(tabID: "tab-1", groupID: group.id, windowID: windowID)
    ...
    func toggleGroupCollapsedTogglesCollapsedState() async throws {
        ...
        try await service.toggleGroupCollapsed(groupID: group.id, windowID: windowID)
        ...
        let groups2 = try await service.getAllGroups(windowID: windowID)
    ...
    func renameGroupChangesGroupName() async throws {
        ...
        try await service.renameGroup(groupID: group.id, newName: "Work Projects", windowID: windowID)
    ...
    func renameGroupPreventsEmptyName() async throws {
        ...
        await #expect(throws: TabGroupError.self) {
            try await service.renameGroup(groupID: group.id, newName: "", windowID: windowID)
        }
    }
    ...
    func changeGroupColorChangesColor() async throws {
        ...
        try await service.changeGroupColor(groupID: group.id, newColor: "#FF0000", windowID: windowID)
