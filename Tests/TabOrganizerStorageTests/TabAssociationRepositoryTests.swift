//
//  TabAssociationRepositoryTests.swift
//  TabOrganizerStorageTests
//
//  Tests for TabAssociationRepository implementations.
...
/// Tests for TabAssociationRepository protocol implementations.
@Suite("TabAssociationRepository Tests")
@MainActor
struct TabAssociationRepositoryTests {

    // MARK: - Test Fixtures

    func makeSampleGroup(name: String = "Work", tabIDs: [String] = ["tab-1", "tab-2"]) -> TabAssociation {
        TabAssociation(
            id: UUID(),
            name: name,
            color: "#0066CC",
    ...
    func saveCreatesNewGroup() async throws {
        ...
        let groups = try await repo.getAllAssociations(windowID: windowID)
    ...
    func saveAllowsSameNameDifferentWindows() async throws {
        ...
        let window1Groups = try await repo.getAllAssociations(windowID: "window-1")
        let window2Groups = try await repo.getAllAssociations(windowID: "window-2")
    ...
    func deleteRemovesGroup() async throws {
        ...
        try await repo.delete(associationID: group.id)
    ...
    func deleteThrowsOnNonExistentGroup() async throws {
        ...
        await #expect(throws: StorageError.self) {
            try await repo.delete(associationID: UUID())
        }
    }

    @Test("Delete only affects target group")
    func deleteOnlyAffectsTargetAssociation() async throws {
        let repo = MockAssociationRepository()
        let windowID = "window-1"
        ...
        try await repo.delete(associationID: group1.id)
    ...
    func getAllAssociationsReturnsEmptyForNewWindow() async throws {
        let repo = MockAssociationRepository()

        let groups = try await repo.getAllAssociations(windowID: "window-1")
        #expect(groups.isEmpty)
    }

    @Test("GetAllGroups returns all groups for window")
    func getAllAssociationsReturnsAllGroupsForWindow() async throws {
        let repo = MockAssociationRepository()
        let windowID = "window-1"
    ...
    func getAllAssociationsIsolatesWindows() async throws {
        let repo = MockAssociationRepository()
    ...
    func getGroupReturnsGroupByID() async throws {
        ...
        let retrieved = try await repo.getAssociation(associationID: group.id)
    ...
    func getGroupReturnsNilForNonExistentID() async throws {
        ...
        let retrieved = try await repo.getAssociation(associationID: UUID())
    ...
    func observeAssociationsEmitsInitialState() async throws {
        let repo = MockAssociationRepository()
        let windowID = "window-1"
        ...
        var receivedGroups: [TabAssociation] = []
        let stream = repo.observeAssociations(windowID: windowID)
    ...
    func observeAssociationsEmitsOnSave() async throws {
        let repo = MockAssociationRepository()
        let windowID = "window-1"
    ...
    func observeAssociationsEmitsOnDelete() async throws {
        let repo = MockAssociationRepository()
        let windowID = "window-1"
        ...
        var latestGroups: [TabAssociation] = []
        let stream = repo.observeAssociations(windowID: windowID)
    ...
    func deleteAllAssociationsRemovesAllGroupsForWindow() async throws {
        let repo = MockAssociationRepository()
        let windowID = "window-1"
        ...
        try await repo.deleteAllAssociations(windowID: windowID)
    ...
    func deleteAllAssociationsOnlyAffectsTargetWindow() async throws {
        let repo = MockAssociationRepository()
        ...
        try await repo.deleteAllAssociations(windowID: "window-1")
    ...
    func getAssociationCountReturnsCorrectCount() async throws {
        let repo = MockAssociationRepository()
        let windowID = "window-1"

        let count1 = try await repo.getAssociationCount(windowID: windowID)
        #expect(count1 == 0)

        let group1 = makeSampleGroup(name: "Work")
        try await repo.save(group1, windowID: windowID)

        let count2 = try await repo.getAssociationCount(windowID: windowID)
        #expect(count2 == 1)

        let group2 = makeSampleGroup(name: "Research")
        try await repo.save(group2, windowID: windowID)

        let count3 = try await repo.getAssociationCount(windowID: windowID)
        #expect(count3 == 2)
    }
}
