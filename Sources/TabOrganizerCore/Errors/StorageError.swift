...
extension StorageError: LocalizedError {
    ...
    public var recoverySuggestion: String? {
        ...
        case .quotaExceeded:
            return "Delete unused tab associations or reduce the number of stored items. Safari extensions have a 5MB storage limit."
        case .notFound:
            return "Verify the resource exists and the identifier is correct."
        case .duplicateName:
            return "Choose a unique name that doesn't conflict with existing items."
        case .unknown:
