...
//  TabOrganizerUI
//
//  Sheet view for creating or editing tab groups.
...
/// View for creating or editing a tab group.
///
/// **Features**:
...
public struct GroupEditorView: View {
    ...
    public init(group: TabGroup?, windowID: String) {
        self.group = group
        ...
        Task {
            do {
                if let existingGroup = group {
                    ...
                    try await state.groupService.updateGroup(
                        updatedGroup,
                        windowID: windowID
                    )
                } else {
...
#Preview("Edit Existing Group") {
    ...
    return GroupEditorView(group: sampleGroup, windowID: "window-1")
        .environment(ExtensionState.preview)
}

#Preview("With Long Name") {
    var longNameGroup = TabGroup(
        id: UUID(),
    ...
    return GroupEditorView(group: longNameGroup, windowID: "window-1")
        .environment(ExtensionState.preview)
}
#endif
