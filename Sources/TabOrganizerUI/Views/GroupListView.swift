...
//  TabOrganizerUI
//
//  Main view displaying all tab groups and their tabs.
...
/// Main view for displaying and managing tab groups.
///
/// **User Story 1 (FR-001 to FR-006)**: Manual tab group organization
...
public struct GroupListView: View {
    ...
    @State private var groupToDelete: TabGroup?
    @State private var showConvertConfirmation: Bool = false
    @State private var groupToConvert: TabGroup?
    ...
    public var body: some View {
        ...
        VStack(spacing: 0) {
            ...
            // Main content
            if state.groupService.isLoading {
                loadingView
            } else if state.filteredGroups.isEmpty && state.ungroupedTabs.isEmpty {
                emptyStateView
            } else {
                ScrollView {
        ...
        .sheet(isPresented: $state.isPresentingGroupEditor) {
            GroupEditorView(
                group: state.groupBeingEdited,
                windowID: state.currentWindowID
            )
        }
        .sheet(isPresented: $state.isPresentingSettings) {
            SettingsView()
        }
        ...
        .alert("Convert to Native", isPresented: $showConvertConfirmation) {
            ...
            if let group = groupToConvert {
                Text("Convert '\(group.name)' to a native Safari tab group? This action cannot be undone.")
            }
        }
        .task {
        ...
        // MARK: - Accessibility
        .accessibilityLabel("Tab groups list")
    ...
    private var headerView: some View {
        HStack(spacing: 12) {
            ...
            HStack(spacing: 8) {
                ...
                TextField("Search groups", text: $state.searchText)
    ...
    @ViewBuilder
    private func groupSection(for group: TabGroup) -> some View {
        Section {
            if !group.collapsed {
                // Tabs in this group
    ...
    private func toggleCollapse(_ group: TabGroup) {
        Task {
            do {
    ...
    private func editGroup(_ group: TabGroup) {
    ...
    private func deleteGroup(_ group: TabGroup) {
        Task {
            do {
                try await state.groupService.deleteGroup(
                    groupID: group.id,
                    windowID: state.currentWindowID
                )
            } catch {
    ...
    private func confirmConvertToNative(_ group: TabGroup) {
        groupToConvert = group
        showConvertConfirmation = true
    }

    /// Converts group to native Safari tab group.
    private func convertToNative(_ group: TabGroup) {
...
#Preview("Empty State") {
    let emptyState = ExtensionState(
        groupService: TabGroupService(
            repository: MockGroupRepository(),
            tabManager: MockTabManager()
        ),
        trackingService: TabTrackingService(storage: MockStorageAdapter())
    )
...
#Preview("Loading State") {
    let loadingState = ExtensionState(
        groupService: TabGroupService(
            repository: MockGroupRepository(),
            tabManager: MockTabManager()
        ),
    ...
    loadingState.groupService.isLoading = true
